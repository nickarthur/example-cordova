...
- (instancetype)init {
    ...
    if (self) {
        ...
        HTMQTTClient *mqttClient = [[HTMQTTClient alloc] initWithHost:HTTransmitterMQTTHost prefix:HTTransmitterTopicPrefix userAgent:HTTransmitterUserAgent logger:self.logger reachability:self.reachability];
...
- (void)endTripWithCompletion:(HTTripBlock)completion {
    ...
    if (!self.locationService.active || !driverID) {
        [self.logger warn:@"No active driver. Cannot end trip."];
        InvokeBlock(completion, nil, [NSError noActiveTripError]);
        return;
    }
    ...
    [self.locationService postLocations:^{
        [self.locationManager requestLocationWithTimeout:locationRequestTimeout
                                                   block:^(CLLocation *currentLocation, HTLocationStatus status) {
                                                       [self endTripWithTripID:driverID
    ...
    HTNetworkRequest *request = [[HTNetworkRequest alloc] initWithMethodType:HTNetworkRequestMethodTypePost
                                                                   APIString:[NSString stringWithFormat:@"api/v1/trips/%@/end/", tripID]
                                                                    ...
                                                                    callback:^(id responseObject, HTNetworkRequestStatus status, NSError *error) {
                                                                        ...
                                                                        [self.locationService updateSDKControls];
                                                                        HTResponse <HTTrip *> *response = [[HTResponse <HTTrip *> alloc] initWithResult:responseObject offline:error.networkError];
                                                                        InvokeBlock(completion, response, nil);
                                                                    }];
    request.cached = YES;
    request.parallel = NO;
