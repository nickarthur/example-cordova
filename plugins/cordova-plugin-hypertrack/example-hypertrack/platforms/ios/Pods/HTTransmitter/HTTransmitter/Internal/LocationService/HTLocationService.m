...
- (void)updateWithSDKControls:(HTSDKControls *)sdkControls updated:(HTSDKControlsUpdate *)update {
    ...
    if (update.minimumDisplacementUpdated) {
    ...
    if (update.minimumDurationUpdated) {
...
- (void)unsubscribeToSDKControlEvents {
    HTNetworkRequest *request = [[HTNetworkRequest alloc] initWithTopicToDelete:[NSString stringWithFormat:@"Push/SDKControls/%@", self.driverID]];
    [self.networkManager processRequest:request];
}
