        ...
        self.requestManager.getSDKControls(userId: userId) { (controls, error) in
            if (error == nil) {
                if let controls = controls {
                    ...
                    HTLogger.shared.info("SDKControls for user: \(userId ) updated to batch_duration: \(controls.batchDuration ?? 0) displacement: \(controls.minimumDisplacement ?? 0) ttl: \(controls.ttl ?? 0)")
