...
final class HTTrackedUserStore {
    ...
    func trackActionFor(collectionId: String, completionHandler: ((_ actions: [HyperTrackAction]?,
        ...
        if(collectionId == trackedUserDataSource.collectionId){
            // CollectionId is already being tracked
            if let completionHandler = completionHandler {
                completionHandler(self.trackedUserDataSource.getActionsList(), nil)
            }
            return
    ...
    @objc private func updateLocations() {
        if let collectionId = trackedUserDataSource.collectionId{
            fetchDetailsFor(collectionId: collectionId, completionHandler: { (users, error) in
